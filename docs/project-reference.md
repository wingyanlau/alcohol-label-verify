...
| Q-INT-08 | **Does COLA hold class/type designation, alcohol content and net contents as structured data?** | Discovered by building, not by analysis: TTB F 5100.31 has **no box for any of the three** — item 15 asks for such information only where it is embossed on the container and absent from the labels. Read off the form's own AcroForm field rectangles while building the region map (D50). So a document-only integration cannot verify three of the four compared fields, and the expected values must come from a database rather than from the filing. This decides whether the product is a document checker or a database comparator, which makes it the **first question to ask**, ahead of every other integration question here. The prototype models the answer optimistically with a synthetic "COLAs Online application record" page; where that page is absent the three fields report `NOT_SUPPLIED` — not assessed, and not a pass | COLA system owner, Sarah |
| Q-INT-07 | **Does measuring what a named agent decided, and how often they departed from the system, trigger a labour-relations notification obligation?** | design §19.6 defers the workforce metric on this question rather than on any engineering one. The data to build it will exist as soon as the agent concept lands (D46), so the decision becomes reachable by accident unless it is asked deliberately. ui-design §2.3 already records labour relations as a stakeholder with, often, a formal obligation for automation affecting bargaining-unit employees | Labour relations, HR |
...
| Q-OPS-08 | **Who approves a change of model, and who is accountable when one changes verdicts?** | No role in the discovery sessions owns this. Sarah owns the process, Marcus the infrastructure, the agents the decisions — and none of them was asked who governs the reader. The prototype records an approval against `IT Systems Administrator`, a role inferred rather than assigned, and leaves the approver unnamed rather than inventing one | Sarah, Marcus, leadership |
| Q-OPS-09 | **How would a repointed model be noticed?** | Vendors publish stable names that move to new weights without notice: `gemini-3.5-flash` has no pinned variant, since numbered pins were retired after 2.0. Every audit record citing it would then be ambiguous, and undetectably so. The prototype fingerprints the reader per job (`model.fingerprinted`) so a change is visible — but nobody owns looking, and a tripwire nobody reads is decoration | Marcus |
| Q-OPS-10 | **Does the firewall permit any external inference endpoint at all?** | Marcus: *"our network blocks outbound traffic to a lot of domains… half their features didn't work because our firewall blocked connections to their ML endpoints."* This is a stated constraint against the architecture staging currently runs — Gemini through an external gateway is two blocked-by-default domains. Workers AI reaches inference through a binding with no outbound call, and an on-premise model would have none either. **The answer decides whether external providers are a real option or only a measurement tool** | **Marcus — ask directly** |
