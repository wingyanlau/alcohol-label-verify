...
|---|---|---|---|
| Verify the three §16.22 formatting rules — bold on the header, absence of bold on the remainder, minimum type size by container volume | Jenny (SRC-5), §16.22 (SRC-6) | Rejected for automated *verification*. The pipeline measures what it can and offers it as evidence; the agent confirms (FR-6a, **D53**) | The original reason given here — that type metrics from a photograph are unreliable and that volume is not in the image — was weaker than the real one, and half of it was wrong: volume *is* application-record data. The governing reason is that the form states TTB **does not routinely review** type size, characters per inch, or contrasting background, placing that duty on the applicant under Part II's certification. An automatic blocking rule would be stricter than the agency's own practice. False rejections carry real cost (N4). See D53 |
...
| D53 | **The §16.22 formatting checks stay advisory permanently, and the pipeline measures instead of merely asking** — type size, characters per inch and separateness are computed and shown as evidence beside the question; the agent still confirms (§3.4, FR-6a) | Promoting them to `automatic` once the geometry existed; leaving all five as unaided questions; or dropping bold as unenforceable | Reading the regulation and the form settled three things, two of which reversed the direction this was heading. **(1) TTB does not check these itself.** The form states it "does not routinely review submitted labels for compliance with applicable requirements for mandatory label information regarding type size, characters per inch, or contrasting background", placing the duty on the applicant under Part II's certification — so an automatic blocking rule would reject labels a COLA specialist would pass, which is a worse failure than silence. Advisory is therefore the correct *permanent* setting, not a staging post. **(2) The regulation states numbers the checklist implied were judgement calls.** §16.22(a)(4) sets 40, 25 and 12 characters per inch at 1, 2 and 3 mm; (b) sets 1/2/3 mm by container volume, and volume is record data the rules may compare. So the pipeline can produce a figure — which is the value, because an agent squinting at artwork is what this replaces. **(3) Bold is law, and the system is silent on it.** (a)(2) requires "capital letters and in bold type" and forbids bold on the remainder; capitals are already enforced (`UT-W05`), bold is checked by neither engine. Three corrections fall out: the checklist's claim that these "cannot be verified from an image" was false for four of five and is fixed; "separate and apart" cites §16.21, not §16.22; and §16.22(c) firmly-affixed — no image can show it — was missing from the checklist entirely. The measurement carries an assumption that must be corrected or it under-measures every reduced label: labels may be shrunk to fit the affix box, with the percentage declared in **item 19**, which is record data and so D4-safe. What stays a bare human judgement, with no figure offered: "readily legible", "contrasting background" — (a)(1) states no threshold, and inventing one while citing the CFR is worse than asking — and firmly-affixed. Bold is shown as a cropped header/remainder pair rather than a claim, and the agent's answer is captured before the pipeline's is revealed: D4 keeps the extractor blind to expected values because anchoring produces false matches, and a reviewer shown the machine's answer first anchors identically | Easy — `automation` is a field in the policy set, and no verdict yet depends on any of it |
| D54 | **The formatting requirements become reference, and the agent's judgement will be captured by the verdict rather than by a checklist** — the checkboxes are removed, the figures and citations stay, and when a measurement earns it the shortfall becomes an advisory *finding* (§7b, D53) | Leaving unenforced checkboxes; enforcing them before any determination; a three-state control per row; or wiring the finding immediately | The checkboxes were built to record what an agent confirmed by eye, and three faults surfaced the moment they were used. **A checkbox is a promise it did not keep** — it looks like something required before proceeding, and nothing required it. **It could express only agreement**: an agent who *saw* the header was not bold had nowhere to put that, so the one outcome the checklist existed to catch was the one it could not record. And **an empty box was ambiguous** between "looked, fine" and "never looked", which is precisely the distinction any confirmation rate would need. Enforcing was the tempting fix and is the worse one: the form states TTB "does not routinely review" these, so compelling an attestation invents an obligation the agency does not impose — the same error as inventing a threshold, aimed at the person instead of the rule — and six compelled ticks per submission become a single sweep of motion within a shift, producing a record that says *confirmed by* a named person about a label nobody examined. False evidence is worse than none, because an audit cannot tell it from the real thing. **What replaces it costs the agent nothing, because the machinery already exists.** A formatting shortfall recorded as a finding at `severity: advisory` makes `needsJudgement` true, which lifts the outcome to `CLEAR_CONFIRM_POLICY` — *some rules need your judgement* (D40) — and the agent's approve, reject or return against that recommendation **is** their answer, captured by `agreementByOutcome` over an index built for exactly this (`decision_by_agreement`). No ticking, no friction, and a better measurement: an act with consequences rather than a box on the way to one. **The entry condition is measurement quality, and it is not met.** On the first staging run 22 of 22 measured submissions read 0.68–0.85 mm against a 2 mm floor, from two independent causes — the model's boxes are estimates (8 px, 10 px, round numbers, roughly 30% under) and the corpus warning is 4.5 pt, genuinely short of 2 mm. Wiring the finding today would make every submission `CLEAR_CONFIRM_POLICY`, the outcome would stop discriminating, and an agent would learn to ignore it. Promotion is `automation: advisory → assisted → automatic` against §5's stated criteria, not a feeling. `decision.advisory_confirmed` is left in place, unused, with this entry as the reason — dropping a column to tidy away a reversal is how a schema stops telling the truth. **Status: the finding route is not built and is not prototype scope.** What is built is the loop it would join — the determination is already recorded against the recommendation, and agreement is already served at `/audit/decisions`. What no part of this system does, or is planned to do here, is *act* on that rate: nothing retrains, no threshold moves, no rule rewrites itself. The claim is only that the data required to do any of it accrues from ordinary work at no cost to the agent (§7b.4) | Easy — the finding is a check kind and a rule, and §18.2 means that arrives as reviewed code |
