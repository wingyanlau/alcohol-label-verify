...
| **US-6** | As Sarah, a result arrives fast enough that agents keep using it | NFR-1 | **Built** | **Batch checks filings as they arrive, so an agent opens prepared recommendations and never waits for a model.** The measured latency is a throughput figure there, not a user-facing one. It applies literally to single review, where a person really is waiting, and the Measurement screen reports it rather than asserting it |
| **US-6a** | As Janet, the batch keeps up with what arrives, so nothing is stale when I open it | — | **Partial** | The pipeline is asynchronous and distributes across invocations, which is the shape that scales. It has not been run at 600/day, and the assumption that filings are visible on arrival is untested against a real system |
